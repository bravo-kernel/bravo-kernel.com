---
title: How to build a fully localized Nextjs application using Lingui v4
...
In this tutorial we will build a fully working localized Nextjs sample application using the latest v4 version of Lingui.js.

After completing the tutorial, you should have a good understanding of the Lingui internationalization/i18n process and a Typescript application with:
...
- Localization files in `*.po` format, ready for super smooth [Crowdin](https://crowdin.com) translations
...
Lingui v4 comes with a lot of improvements that make setting up localization a lot easier. For example,
...
- Is the single source of truth for all our Lingui/Nextjs localization settings
...
- Make `lingui.config.js` the single source of truth for Nextjs localization
...
![Lingui extract command line result](/static/images/blog/2023/05/how-to-build-a-fully-localized-Nextjs-application-using-Lingui-v4/lingui-extract-result.png)
...
or your pages might not update in realtime (e.g. when using the language switcher) 
...
- Try the free version of [Crowdin](https://crowdin) which provides more than sufficient functionality for small(er) projects and includes one Github integration for automated translations via Pull Requests. A description of the Crowdin workflow can be [found here](https://github.com/lingui/js-lingui/blob/next/website/docs/tools/crowdin.md).
